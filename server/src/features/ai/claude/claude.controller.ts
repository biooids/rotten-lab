...
import { DEFAULT_CLAUDE_MODEL, VALID_CLAUDE_MODELS } from "./claude.config.js";
...
export const claudeController = {
  ...
  async scanUrl(req: IncomingMessage, res: ServerResponse): Promise<void> {
    ...
    let resolvedModel: ClaudeModelId = DEFAULT_CLAUDE_MODEL;
    if (body.model && VALID_CLAUDE_MODELS.includes(body.model as any)) {
  ...
  // --- ADD THIS NEW METHOD TO claudeController ---
  async cancelScan(
    req: IncomingMessage,
    res: ServerResponse,
    reportId: string,
    ...
    res.setHeader("Content-Type", "application/json");
    process.stdout.write(
      `\n[HTTP] POST /api/v1/ai/claude/report/${reportId}/cancel initialized\n`,
    );
