-- --- 1. NUCLEAR CLEANUP ---
DROP TABLE IF EXISTS report_chats CASCADE;
...
-- --- 5.6. THE REPORT CHATS TABLE (NEW) ---
CREATE TABLE report_chats (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    report_id UUID NOT NULL REFERENCES scan_reports(id) ON DELETE CASCADE,
    user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    role chat_role NOT NULL,
    -- Strict limits: Message cannot be empty, and cannot exceed 8192 chars (prevents payload bloat but allows code snippets)
    message TEXT NOT NULL CHECK (char_length(trim(message)) > 0 AND char_length(message) <= 8192),
    created_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP
);
...
-- Chat Optimization Index (Compound index for fast chronological sorting per report)
CREATE INDEX idx_report_chats_report_id_time ON report_chats(report_id, created_at ASC);
...
CREATE TRIGGER trigger_update_scan_reports_timestamp
    ...
    FOR EACH ROW EXECUTE FUNCTION set_updated_at();-- --- 1. NUCLEAR CLEANUP ---
DROP TABLE IF EXISTS report_chats CASCADE;
DROP TABLE IF EXISTS ai_token_logs CASCADE;
DROP TABLE IF EXISTS scan_findings CASCADE;
DROP TABLE IF EXISTS scan_reports CASCADE;
