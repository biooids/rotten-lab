...
-- --- 5.4. THE AI SCAN FINDINGS TABLE ---
...
-- --- 5.6. THE REPORT CHATS TABLE (UPDATED: Added finding_id) ---
CREATE TABLE report_chats (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    report_id UUID NOT NULL REFERENCES scan_reports(id) ON DELETE CASCADE,
    -- NEW: Strictly bind this chat message to a specific vulnerability finding
...
-- Chat Optimization Index (UPDATED: Sort chat by finding_id instead of report_id)
CREATE INDEX idx_report_chats_finding_id_time ON report_chats(finding_id, created_at ASC);
