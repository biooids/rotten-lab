...
export default function UserDirectory({ currentUserId, showToast }: Props) {
  ...
  const handleAiAccessChange = async (
    userId: string,
    hasAccess: boolean,
    currentPreferSystem: boolean,
  ) => {
    ...
    try {
      await updateAiAccess({
        targetUserId: userId,
        hasAccess,
        preferSystemKey: hasAccess ? currentPreferSystem : false,
      }).unwrap();
                        ...
                        <div className="flex items-center gap-3 h-8 w-28">
                          {isCurrentlyUpdatingAi ? (
                            ...
                            <Select
                              ...
                              value={u.has_system_ai_access ? "true" : "false"}
                              onValueChange={(val) =>
                                handleAiAccessChange(
                                  u.id,
                                  val === "true",
